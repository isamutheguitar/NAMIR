 ...
 */
const guessFromFilename = (filename: string) => {
    ...
    // 5. モデラー検出 (Axe FX, Kemper 등)
    let modeler = '';
    const axeMatch = name.match(/axe[\s-]*fx(?:[\s-]*(ii|iii|2|3))?/i);
    if (axeMatch) {
        modeler = `Axe FX ${axeMatch[1] ? axeMatch[1].toUpperCase() : ''}`.trim();
    } else {
        ...
        if (otherModelerMatch) {
            ...
            if (m.includes('helix')) modeler = 'Helix';
            else if (m.includes('kemper')) modeler = 'Kemper';
            else if (m.includes('tonex')) modeler = 'Tonex';
            else modeler = 'Quad Cortex';
        }
    }
    ...
    return {
        amp: amp, // ★修正: 無理に最初の単語を入れず、確実なマッチだけを返す
        fallbackAmp: name.split(/[_\-\s+]+/)[0] || 'Unknown', // ★保険として最初の単語は別枠で持つ
        pedal,
        cab,
        mic,
        tone,
 ...
 */
export const parseNamMetadata = (content: string, filename: string): NamMetadata => {
    try {
        ...
        } else if (searchContext.includes('pedal') || (guess.pedal && !guess.amp)) {
            capture_type = 'Pedal Only';
        } else if (searchContext.includes('amp') || searchContext.includes('preamp')) {
            capture_type = 'Amp Only';
        }

        // ★修正: ペダル単体の場合はペダル名をメインとし、アンプと併用の場合はアンプ名の後ろにカッコで併記する
        let finalMakeBase = guess.amp ? guess.amp : (!isInvalid(raw_make) ? raw_make : guess.fallbackAmp);
        if (capture_type === 'Pedal Only' && guess.pedal) {
            finalMakeBase = guess.pedal;
        } else if (guess.pedal) {
            finalMakeBase = `${finalMakeBase !== 'Unknown' ? finalMakeBase : ''} (${guess.pedal})`.trim();
        }

        // ★モデラー情報があればアンプ名の末尾に付与する（Axe FX, Kemper等）
        const finalMake = guess.modeler ? `${finalMakeBase !== 'Unknown' ? finalMakeBase : ''} ${guess.modeler}`.trim() : finalMakeBase;

        const validRawModel = !isInvalid(raw_model) ? raw_model : '';
        const finalModel = validRawModel || 'Unknown'; // ペダルはアンプ欄に組み込まれたのでModelからは外す
