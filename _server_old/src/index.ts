...
app.post('/api/auth/google', async (req, res) => {
    ...
    try {
        ...
        console.log('Googleから取得したemail:', receivedEmail);
        console.log('環境変数から読み込んだ許可email:', authorizedEmail);

        if (receivedEmail !== authorizedEmail) {
            console.log('判定結果: 一致しませんでした (Access Denied)');
            res.status(403).json({ error: 'Access denied: Unauthorized email address' });
            return;
        }
        console.log('判定結果: 一致しました (Access Granted)');
...
app.post('/api/save-to-sheet', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/get-library', authenticateToken, async (req, res) => {
    try {
        const spreadsheetId = process.env.GOOGLE_SHEET_ID;
        if (!spreadsheetId) {
            res.status(500).json({ error: 'Google Sheet ID is not configured' });
...
app.listen(PORT, HOST, () => {
    ...
    console.log(`Local network access via your IP address, e.g., http://<your-ip>:${PORT}`);
});
